...
warnings.filterwarnings('ignore')"""),
        ...
        nbf.v4.new_code_cell("""# Cargar el dataset real
...
df.head()"""),
        ...
        nbf.v4.new_markdown_cell("## 2. Aprendizaje No Supervisado: Agrupación de Equipos (Clustering)\nVamos a agrupar a las selecciones en 'Tiers' (Niveles) usando **K-Means** basándonos en su rendimiento histórico (Goles promedio y Ratio de victorias)."),
        nbf.v4.new_code_cell("""# Calcular estadísticas por equipo (como Local)
...
plt.show()"""),
        ...
        nbf.v4.new_markdown_cell("## 3. Preparación de Datos para el Modelo Supervisado"),
        nbf.v4.new_code_cell("""# Unir los clusters al dataframe original como nuevas características
...
y_away = df['away_score_capped']"""),
        ...
        nbf.v4.new_markdown_cell("## 4. Aprendizaje Supervisado: Entrenando Random Forest\nAquí usamos las respuestas correctas (goles reales) para predecir futuros partidos."),
        nbf.v4.new_code_cell("""preprocessor = ColumnTransformer(
    transformers=[
...
print("¡Entrenamiento completado!")"""),
        ...
        nbf.v4.new_markdown_cell("## 5. Predicción (Interfaz Interactiva)\nCambia los equipos en la celda de abajo y ejecútala para ver el resultado más probable."),
        nbf.v4.new_code_cell("""# ¡Prueba tú mismo!
EQUIPO_A = "Argentina"
EQUIPO_B = "France"
...
print(f"🎯 Resultado más probable: {EQUIPO_A} {pred_h} - {pred_a} {EQUIPO_B} ({prob_max*100:.1f}%)")""")
    ...
    output_path = os.path.join(os.path.dirname(__file__), '..', 'notebooks', '02_modelado_y_prediccion.ipynb')
    with open(output_path, 'w', encoding='utf-8') as f:
        nbf.write(nb, f)
    
    print(f"Notebook creado en: {os.path.abspath(output_path)}")
